...
from config import TRAIN_REVIEW, TRAIN_LABEL, TRAIN_DATASET, TEST_DATASET, DEV_DATASET
...
#             sent = row[1]
#             netags = netagger(sent, sent_id)
#             sents_test.append((sent_id, [(char, tag) for char, tag in zip(sent, netags)]))   
#         print(sents_test[0])

sents_dev = sents_train[:int(len(sents_train) * 0.15)]
sents_test = sents_train[0:1]
sents_train = sents_train[int(len(sents_train) * 0.15):]
